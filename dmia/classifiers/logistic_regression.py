...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            batch = np.random.choice(dim, batch_size)
            X_batch = X[batch]
            y_batch = y[batch]
            ...
            self.w -= gradW * learning_rate / num_iters
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        h_theta_0 = self.logistic_function(X.dot(self.w.T))
        h_theta_1 = 1 - h_theta_0
        y_proba = np.vstack((h_theta_1, h_theta_0)).T
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        h_theta = self.logistic_function(X_batch.dot(self.w.T))
        m = 1 / X_batch.shape[0]
        loss = - m * (np.dot(y_batch, np.log(h_theta)) + np.inner(1 - y_batch, np.log(1 - h_theta)))
        dw = m * (h_theta - y_batch) * X_batch

        # Right now the loss is a sum over all training examples, but we want it
        # to be an average instead so we divide by num_train.
        # Note that the same thing must be done with gradient.

        loss += reg * m / 2 * np.sum(self.w ** 2)
        dw += reg * m * self.w
